...
'''
str2 = "turo is a good man! turo is a nice man! turo is a handsome man"
# d = re.finditer(r"(turo)", str2)
# while True:
#     try:
#         l = next(d)
#         print(l)
...
参数:
    ...
    count: 最多替换次数
功能:
    在目标字符串中以正则表达式的规则匹配字符串, 再把他们替换
    成指定的字符串, 可以指定替换的次数, 如果不指定,
    则替换所有匹配字符串.
区别:
    前者返回一个被替换的字符串, 后者返回一个元组, 第一个
    元素是被替换的字符串, 第二个元素是被替换的次数


'''
str5 = "turo is a good good good man!"
...
print(re.match(pat, "13600000000"))# 原来写法
#正则对象命名
re_telephon = re.compile(pat)
print(re_telephon.match("13600000000")) #现写法
